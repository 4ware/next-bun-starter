...
Todos can carry one image. Storage is in Postgres: `uploads` table (`bytea` via a drizzle `customType`) + nullable `todos.image_id` (FK, `set null`). `POST /api/todos/:id/image` accepts multipart (`t.File` validated to jpeg/png/webp/gif, max 5 MB), inserts the new upload, points the todo at it, deletes the replaced upload, and revalidates the todos cache tag; deleting a todo also deletes its upload. `GET /api/uploads/:id` serves the bytes user-scoped with `Cache-Control: private, immutable` (upload ids are immutable — replacing creates a new row). Client: `src/components/todos/todo-panel.tsx` wraps the list in `<Uploady>` with a per-row `asUploadButton` trigger (per-todo `destination`); a global `useItemFinishListener` invalidates the todos query + toasts, `useItemErrorListener` surfaces the API's `{ error }`. Thumbnails use plain `<img>`, not next/image — the route needs the session cookie, which the image optimizer would not forward.
