...
Route validation uses **zod, not TypeBox**: Elysia 1.4 accepts Standard Schema validators directly in `body`/`params`, and handlers receive the *parsed* zod output (unknown keys stripped). Todo schemas live in `src/lib/validators/todos.ts` and are **derived from the drizzle table via drizzle-zod** (`createInsertSchema`/`createUpdateSchema`/`createSelectSchema`), `.pick()`ing only form-submitted fields (`title`, `done`; params pick `id`, which maps to real uuid validation) with length rules layered on via the refinement argument — column changes propagate into validation automatically. The upload `z.file()` schema and the realtime message schema stay hand-written (not DB-backed). The global `onError` in `src/server/api/index.ts` normalizes validation failures to `{ error: string }` (422), reading zod's `message` (with a fallback to TypeBox's `summary`). **Validator messages are localized**: schemas emit stable keys from the `ApiValidation` namespace in `messages/{en,de}.json` (e.g. `"titleRequired"`), and `onError` translates them via `src/server/api/validation-messages.ts` using the request's `NEXT_LOCALE` cookie (set by the next-intl proxy) with `Accept-Language` fallback; unknown messages pass through untranslated. Test caveat: happy-dom's `Request` drops the forbidden `Cookie` header — cookie-dependent API tests use `NativeRequest` from `src/test/setup.ts`.

### Realtime WebSockets

Next.js route handlers cannot upgrade to WebSocket, so the realtime Elysia app (`src/server/realtime/`) runs as its own Bun process (`bun run dev:realtime`, port `REALTIME_PORT`, default 3001). It shares the better-auth instance: the browser sends the session cookie on the upgrade request (same host, different port), an instance-level `.resolve()` rejects the handshake with 401 when there is no session (plus a 403 origin check), and `ws.data.user` is typed in the ws handlers. Wire types live in `src/lib/realtime.ts`, shared by server and client; incoming ws messages are validated against the zod `realtimeClientMessageSchema` defined there (`RealtimeClientMessage` is inferred from it). The app is built by `createRealtime(resolveUser?)` — tests inject a fake session resolver instead of mocking the auth module (`realtime.test.ts` drives a real socket via `NativeWebSocket` from `src/test/setup.ts`, captured before happy-dom replaces the global). The client (`src/components/realtime/live-panel.tsx`) reads `env.NEXT_PUBLIC_REALTIME_URL` from `src/lib/env.ts`. Note: the HTTP API and realtime server are separate processes, so HTTP routes can't publish to WS clients without a broker.
